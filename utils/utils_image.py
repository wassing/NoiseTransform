...
def rgb2ycbcr_LT(img, only_y=True):

    img = img.data.squeeze().float().clamp_(0, 1).cpu().numpy()
    if img.ndim == 3:
        img = np.transpose(img, (1, 2, 0))
    if only_y:
        image = np.dot(img, [65.481, 128.553, 24.966]) + 16.0
    else:
        image = np.matmul(img, [[65.481, -37.797, 112.0], [128.553, -74.203, -93.786], [24.966, 112.0, -18.214]]) + [16, 128, 128]
